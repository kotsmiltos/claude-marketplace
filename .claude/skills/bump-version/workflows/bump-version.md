...
| Core files (Phase 1 sanity check) | `types.ts`, `state.ts`, `runner.ts`, `runner.test.ts`, `paginate.ts`, `paginate.test.ts`, `define-config.ts`, `index.ts` + the module dirs `compile/`, `run/`, `interaction/`, `controls/`, `handoff/` (tree since 2.0.0) | `index.ts`, `run-tracer.ts`, `event-emitter.ts`, `schemas.ts`, `kafka-producer.ts`, `bounded-queue.ts`, `redaction.ts`, `settings.ts`, `env.ts`, `registry.ts`, `event-producer.ts` (+ their `.test.ts` files, `README.md`) |
...
1. **Verbatim replace** every Tier-1 file from the staged source into the target's embedded copy
   (per the target table). For `agent-step` the library is a tree (recursive since 2.0.0) — wipe
   first so removed files don't linger, and exclude any source-repo-only notes like `UPSTREAM.md`:
   ```
   ...
   ```
   Mirror any added/removed library files into the docs that enumerate them — `agent-step`: the
   bootstrap copy list (`workflows/bootstrap-project.md` Step 5 + file-list),
   `create-tool/SKILL.md` `templates_index`, and this skill's `references/tracked-assets.md`
   Tier-1 inventory; `observability`: the vendored-file count/inventory in
   `workflows/add-kafka-observability.md` and the library `README.md`.
