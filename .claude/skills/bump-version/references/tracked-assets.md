...
- `skills/create-tool/references/orchestration-boundaries.md` — the action/graph/library boundary doctrine: no wrapper actions, executor-written terminal handoffs (`createTerminalHandoff`), the three-node graph target, the forbidden confirmation nudge, handoff honesty. Update when the library absorbs a mechanism a host used to hand-roll (the doctrine names what the library now owns), or when a new library capability deletes a host-side pattern this doc calls an anti-pattern.
