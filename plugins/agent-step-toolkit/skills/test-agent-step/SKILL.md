...
4. **Run them** with the project's scripts: `npm test` (library unit tests — fast, scoped to `dist/agent-step/*.test.js`: runner, paginate, capture, capture-bounce, handoff, hardening, repeat-question, note-refusal, dictation, verdict-map, action-describe, gate-contract, zod-state), `npm run test:sandbox` (per-tool sandbox tests; needs the local sandbox up), `npm run test:prompt` (prompt-input; sets `PROMPT_INPUT_LIVE=1`). The sandbox/prompt scripts discover compiled test files via a guarded `find`, so they only run the layer they name.
