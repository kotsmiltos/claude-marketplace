...
- **mutation?** — if yes: pick `soleStep` (strict alone) or `soleOnExecute` (propose may ride; execute alone); plus `requiresConfirmation` opts (`maxAttempts` default 3; `lockdown` default true; no TTL — the runner never times gates out; `readBack` to render what the runner recorded onto the proposal as `read_back` — add it whenever the params carry caller-dictated digits; `readBackDirective` (≥ 3.0.0) for the framing that rides beside it; `repeatReadBack` (≥ 2.5.0) to persist that rendering so the sole-step `repeat_pending_question` control can return the exact stored bytes on a "pardon?" turn — since 3.0.0 it REQUIRES a `replyContract`; `replyContract` (≥ 3.0.0, a `GateContractSpec`) when the gate's reply taxonomy outranks the generic yes/correction contract; `refuseProposal` (≥ 2.5.0) for propose-time state-aware refusal — a schema-valid but state-impossible proposal is answered immediately, storing and burning nothing) if confirm-required
- **verdicts** — the declared rows (≥ 3.0.0, REQUIRED for every outcome the executor names): per code — `ok` flag, summary (static or a `(state, data)` renderer), body fields in wire order, static `stateUpdate`, effects (a terminal verdict carries its `request_handoff` effect HERE), `backendFailure` for codes the auto-handoff counter should see
- **asks?** — standing asks keyed by verdict-or-error code (≥ 3.0.0): which action/param the caller's next value feeds, `expects` digits/text, optional `render` for engine-rendered `ask_text`
- **captureBounces?** — `{ max: 2, ladder }` (≥ 3.0.0) to bound consecutive `invalid_params` re-asks of a dictated capture; requires the named ladder in `ladders`
...
1. `src/tools/<name>/names.ts` — the types-only `ActionName` / `PrereqName` leaf (see `templates/names.ts.template`)
2. `src/tools/<name>/backend/env.ts` — env constants (see `templates/backend-env.ts.template`)
3. `src/tools/<name>/backend/client.ts` — HTTP helper (see `templates/backend-client.ts.template`; adapt to backend protocol)
4. `src/tools/<name>/shared/` — any shared helpers (e.g. resolve-entity, a `rows.ts` verdict-row vocabulary shared across actions); create only if multiple actions need them. (Pagination needs no shared helper — it's a library opt; see step 6.)
5. `src/tools/<name>/verifiers/<prereq>.ts` — one file per unique prereq (see `templates/verifier.ts.template`)
5b. `src/tools/<name>/actions/<action>/stateSelector.ts` — one per action; exports `getSlice` + `Slice` (see `templates/state-selector.ts.template`). Narrow to the slots the action reads.
5c. `src/tools/<name>/actions/<action>/action.ts` — one per action; the DECLARATION from the plan: description (semantic lead ONLY — the engine composes the gate marker; never restate the handshake or enumerate verdicts), paramsSchema (capture builders for caller-dictated params), prereqs, the `verdicts` rows (static summaries/doctrine/state writes; a terminal row carries its `request_handoff` effect), `asks`, `captureBounces`, `controller`, `invalidatesOnChange` (see `templates/action.ts.template`; never import `./executor.js`)
6. `src/tools/<name>/actions/<action>/executor.ts` — one per action; imports its `Slice` from `./stateSelector.js`, returns `DeclaredExecutorResult` naming the declared rows. Pick by shape: `templates/executor-read.ts.template` (fixed-size read), `templates/executor-read-paginated.ts.template` (large/list read — declare `pageable` on the action with a `z.object` params schema; the runner injects `page`/`pageSize`, slices, and caches in the library `pagedRead` slot — no per-tool state slot or cache helper needed), `templates/executor-mutation.ts.template` (mutation), `templates/executor-handoff.ts.template` (channel handoff — see step 6c), or `templates/executor-analysis.ts.template` (LLM-authored compute over fetched data — see step 6b).
...
6c. **Channel-handoff action (only if the tool transfers the caller to other services/agents).** Follow `references/streaming-and-channel-contract.md` (incl. its `<agent_roles>` section) and add, alongside the other actions:
   ...
   - **The agent's role (a bootstrap fact) picks the mechanism.** ORCHESTRATOR: the scaffold tool action below — targets are the specialized agents (agent-name `serviceType`s) plus any client-side types, outbound routing, spoken transition. SPECIALIZED (agent-step ≥ 1.3.0): prefer the **library built-in** — no per-tool action at all; opt in via `buildAgentStepTool({ handoff: spec })`, write the env→spec module, and wire the `resolve_handoff` node + conditional edge into a hand-rolled graph (see `agent-step-api.md` `<handoff>`); this also buys delegate mode (route an off-topic turn to a general agent and KEEP the conversation). For an honest `completed`/`abandon` closing, set `spec.resolveClosingMessage(state, request)` (agent-step ≥ 1.5.0) so the spoken line reflects whether the mutation actually persisted — return the success line only when state proves it, else a neutral/failed phrasing (`undefined` falls through to the LLM-composed `context`). Two further spec hooks (agent-step ≥ 2.3.0) keep host logic out of the frozen envelope: `resolveHandoffType` decides the SIGNAL from state before the first control-plane event; `resolveHandoffMetadata` merges host fields into `handoff_metadata` (library keys still applied last). Terminality is declared on verdict rows (≥ 3.0.0): a terminal business outcome arms its handoff atomically via the row's `request_handoff` effect — there is no state-predicate net (`forcedHandoff` was removed); input-derived terminality converts to a first-turn probe action whose row carries the terminal effect. For a voice/chat agent whose callers drift into small talk, configure an aside LADDER in `ladders` (≥ 3.0.0): the free in-place deflection per task of an aside NO configured agent serves, escalating into the `off_topic` handback on exhaustion — the prompt teaches only the split between topics other agents serve (still `request_handoff` + `off_topic`) and nobody-serves-it chit-chat (`note_refusal` with the aside ladder). When the fleet's terminal routes are a fixed set of reason/context pairs, set `HandoffSpec.modelRequestSchema` (≥ 2.5.0) so the pairs are configuration the engine renders and parses, not prompt prose — and leave ladder escalation contexts OUT of it, so the engine holds the only path to them. The scaffold alternative for a specialized agent that stays on `createReactAgent` and needs only signals: a handback tool action with params `{ signal: z.enum(["completed","abandon","off_topic"]), reason: z.string() }`, the executor mapping `serviceType` = the signal value, with a per-signal `successMessage` (the completed wrap-up line, the abandon acknowledgement, the off_topic "let me get you back" line).
   ...
   - The handoff action's executor ← `templates/executor-handoff.ts.template`. The action's ok ROW MUST carry `isHandoff: true` in its declared body (the bootstrap `agent.ts` hook keys off it) plus `successMessage`; the executor's `stateUpdate` writes the bootstrap `pendingHandoff` slot (already in `state.ts` — do not redeclare). Identity fields are coerced `?? null`.
   - Declaration (`action.ts`): `controller: { soleStep: true }`; params `{ service: z.enum([...catalog names]), reason: z.string() }` (orchestrator) or the `{ signal, reason }` shape above (specialized).
...
7. `src/tools/<name>/config.ts` — the assembler: imports each `actions/<action>/action.js` declaration into `defineConfig<ActionName, PrereqName, State>` (see `templates/config.ts.template`)
8. `src/tools/<name>/index.ts` — wire-up: `selectors` (`satisfies SelectorRegistry<State, ActionName>`) + `executors` (`ExecutorRegistry<State, typeof selectors>`), both keyed by action name, plus the optional features (`handoff`, `ladders`, `abortPolicy`, `messages`) (see `templates/tool-index.ts.template`)
...
11. `src/prompt.ts` — wire the `{PROTOCOL}` splice if this is the project's first tool (`composeProtocolPrompt` over the config + ladders + handoff spec; domain capture doctrines go on `toolTurnRules`), then add semantic ACTIONS block(s) — prompt structure per `references/agent-design-principles.md` and `references/state-and-prompt-integration.md` (one authority per behaviour; the machinery text is engine-composed; tool results as the sole state authority)
...
```
Expected: all library unit tests still pass (runner + paginate + capture + capture-bounce + handoff + hardening + repeat-question + note-refusal + dictation + verdict-map + action-describe + gate-contract + zod-state). The count may grow as the library evolves — what matters is zero failures. The new tool shouldn't affect them.
