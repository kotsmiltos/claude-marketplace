...
```
Expected: all library unit tests still pass (currently 173: runner + paginate + capture + handoff + bounded-choice + hardening + zod-state). The count may grow as the library evolves — what matters is zero failures. The new tool shouldn't affect them.
