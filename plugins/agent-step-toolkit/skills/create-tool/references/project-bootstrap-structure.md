...
- **types.ts** — the authoring contracts: `AgentStepConfig` (`{ tool, actions }`), `ActionDef` (with `verdicts` rows, `asks`, `captureBounces`, inline `controller: ControllerHooks` + `invalidatesOnChange`), `VerdictDef` + `DeclaredExecutorResult` (the declarative executor return), `ControllerHooks` (batch-shape opts + confirmation / OTP / match / flow lifecycle opts), `ConfirmationOpts` (+ `GateContractSpec`), `DictationAsk`, `Verifier`, `ExecutorEffect` (the typed library-state transitions a row or executor may request).
- **state.ts** — the library-managed state slots: `AwaitingInputSchema` (discriminated union over dictation / confirmation / otp / match) → `AwaitingInput`, `CurrentFlowSchema` → `CurrentFlow`, `PagedCacheSchema`, `HandoffRequestSchema` → `HandoffRequest` (the library handoff request slot), the `spentLadders` once-latch counts; the slot metadata table `AGENT_STEP_SLOT_META`, the `agentStepZodShape` fragment hosts spread into their Zod state schema (each slot wrapped with `withLangGraph`, carrying its reducer/default), the equivalent `agentStepStateSpec` Annotation fragment (legacy/Annotation path), and the derived views `agentStepInternalSlotMask` / `agentStepTaskScopedSlots` / `agentStepRunnerOwnedSlots`.
...
- **compile/** — `validate.ts` (every construction-time check, incl. cross-action config pairs, verdict/ask/ladder validation, and state-schema channel completeness), `plan.ts` (`BuildAgentStepToolOptions` + the compiled plan), `activation.ts` (the single ControlActivation construction), `schema.ts` / `describe.ts` (the model-facing schema + tool description), `action-describe.ts` (the composed per-action description), `gate-contract.ts` (`composeGateContract`), `protocol.ts` (`composeProtocolPrompt`), `state-schema.ts` (`StateSchemaLike` resolution).
- **run/** — `admission.ts` (fixed-order whole-batch preconditions), `planning.ts` (confirm-mode freeze = the same-batch-bypass safety property), `execution.ts` (per-step loop, verdict-row normalization, effects ordering, handoff monotonicity), `finalize.ts` (result body + standing-ask lifecycle + error-counter/auto-handoff policy), `batch-state.ts` (the single write path into view/committed), `value-equal.ts` (canonical value equality).
- **interaction/** — one policy module per interaction kind: `confirmation.ts`, `otp.ts`, `match.ts`, `flow.ts`, `dictation.ts` (the standing-ask lifecycle), `turn-identity.ts` (caller-turn identity resolution, `resolveCallerTurnId`).
- **controls/** — the library-owned model-facing actions (`abort.ts`, `request-handoff.ts`, `repeat-question.ts`, `note-refusal.ts` — also the `climbLadder` core + ladder types) as `ControlAction` implementations in an ordered `registry.ts`, over the shared `contract.ts`.
- **handoff/** — library-coordinated channel handoff for specialized agents: `contract.ts` (`HandoffSpec`, `HANDOFF_ACTION`, `HANDBACK_SIGNALS`, `handoffRequested`), `node.ts` (`createHandoffNode` — terminate envelope or delegate to another LangGraph deployment with live token pass-through), `delegate-client.ts` (SSE/Platform-API transport). Opt-in; see `streaming-and-channel-contract.md`.
- **messages.ts** — the runner's overridable system `summary` strings + the model-facing wire contracts (neutral English defaults).
- **define-config.ts** — identity helper for typed `defineConfig({...})` calls.
- **index.ts** — the public surface (re-exports; only what's here is API). Includes `StateSchemaLike` (the `stateSchema` option's type: an Annotation OR a Zod object), the composers (`composeGateContract`, `composeProtocolPrompt`), the ladder types, and the slot fragments/views.
- **capture.ts** — caller capture primitives for tool param authoring (`digitsOnly`, `callerDigits`, `digitGroupsParam`, `digitCandidatesParam`, `callerTextParam`, `relayParam`, `exactlyOneOf`); see `agent-step-api.md` `<caller_digit_capture>`.
- **runner.test.ts**, **handoff.test.ts**, **hardening.test.ts**, **paginate.test.ts**, **capture.test.ts**, **capture-bounce.test.ts**, **repeat-question.test.ts**, **note-refusal.test.ts**, **dictation.test.ts**, **verdict-map.test.ts**, **action-describe.test.ts**, **gate-contract.test.ts**, **zod-state.test.ts** — unit tests covering every runner branch, the handoff machinery, the hardening guards, the pagination primitives, the capture primitives, the capture-bounce bound, the stored-byte question repeat, the ladder mechanism, the standing-ask lifecycle, the declared-verdict normalization, the composed descriptions/contracts, and the Zod-schema merger derivation. Run via `npm test`.
