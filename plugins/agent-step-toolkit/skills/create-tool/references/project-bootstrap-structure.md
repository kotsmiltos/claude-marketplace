...
- **interaction/** — one policy module per interaction kind: `confirmation.ts`, `otp.ts`, `match.ts`, `flow.ts`, `bounded-choice.ts` (each states its lockdown, attempts, freshness, and clearing rules in one place; `bounded-choice.ts` also owns caller-turn identity resolution).
- **controls/** — the library-owned model-facing actions (`abort.ts`, `request-handoff.ts`, `bounded-choice.ts`) as `ControlAction` implementations in an ordered `registry.ts`.
