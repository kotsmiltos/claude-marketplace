...
3. **Admission** (`run/admission.ts`) — every whole-batch precondition, in a FIXED order, each able to refuse the batch (a single result entry, `failed_at: 0`, nothing committed):
   ...
   b. **Gate lockdown** — a pending confirmation/OTP/match admits only its target action / abort / handoff / the repeat control (+ the capturer, for match; see lockdown table above).
