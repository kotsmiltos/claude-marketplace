...
Schema helpers for params that carry **caller-dictated digits** (a tax number, a card-number tail, an OTP-adjacent code). The runner does not call them, but they encode invariants the runner imposes — use them instead of hand-rolling, because every hand-rolled variant has re-created at least one of these live failures:
